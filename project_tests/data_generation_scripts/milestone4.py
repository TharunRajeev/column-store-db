...
def generateDataMilestone4(
    ...
    outputFactTable.to_csv(
        outputFile1, sep=",", index=False, header=header_line_fact, lineterminator="\n"
    )
    outputDimTable1.to_csv(
        outputFile2, sep=",", index=False, header=header_line_dim1, lineterminator="\n"
    )
    outputDimTable2.to_csv(
        outputFile3, sep=",", index=False, header=header_line_dim2, lineterminator="\n"
    )
    outputSelectTable1.to_csv(
        outputFile4, sep=",", index=False, header=header_line_sel1, lineterminator="\n"
    )
    outputSelectTable2.to_csv(
        outputFile5, sep=",", index=False, header=header_line_sel2, lineterminator="\n"
    )
    return (
        outputFactTable,
        outputDimTable1,
...
def createTest46(
    factTable, dimTable2, dataSizeFact, dataSizeDim2, selectivityFact, selectivityDim2
):
    output_file, exp_output_file = data_gen_utils.openFileHandles(
        46, TEST_DIR=TEST_BASE_DIR
    )
    ...
    output_file.write(
        "-- SELECT avg(tbl5_fact.col2), sum(tbl5_fact.col3) FROM tbl5_fact,tbl5_dim2 WHERE tbl5_fact.col4=tbl5_dim2.col1 AND tbl5_fact.col2 < {} AND tbl5_dim2.col1<{};\n".format(
            int((dataSizeFact / 5) * selectivityFact),
            int(selectivityDim2 * dataSizeDim2),
        )
    )
    output_file.write("--\n")
    output_file.write("--\n")
    output_file.write(
        "p1=select(db1.tbl5_fact.col2,null, {})\n".format(
            int((dataSizeFact / 5) * selectivityFact)
        )
    )
    output_file.write(
        "p2=select(db1.tbl5_dim2.col1,null, {})\n".format(
            int(dataSizeDim2 * selectivityDim2)
        )
    )
    ...
    joinedTable = preJoinFact.merge(
        preJoinDim2, left_on="col4", right_on="col1", suffixes=("", "_right")
    )
...
def createTest48(
    factTable, dimTable1, dataSizeFact, dataSizeDim1, selectivityFact, selectivityDim1
):
    output_file, exp_output_file = data_gen_utils.openFileHandles(
        48, TEST_DIR=TEST_BASE_DIR
    )
    ...
    output_file.write("-- Query in SQL:\n")
    output_file.write(
        "-- SELECT sum(tbl5_fact.col2), avg(tbl5_dim1.col1) FROM tbl5_fact,tbl5_dim1 WHERE tbl5_fact.col1=tbl5_dim1.col1 AND tbl5_fact.col2 < {} AND tbl5_dim1.col3<{};\n".format(
            int(selectivityFact * (dataSizeFact / 5)),
            int((dataSizeDim1 / 5) * selectivityDim1),
        )
    )
    output_file.write("--\n")
    output_file.write("--\n")
    ...
    output_file.write(
        "p2=select(db1.tbl5_dim1.col3,null, {})\n".format(
            int((dataSizeDim1 / 5) * selectivityDim1)
        )
    )
    ...
    joinedTable = preJoinFact.merge(
        preJoinDim1, left_on="col1", right_on="col1", suffixes=("", "_right")
    )
...
def createTest50(
    ...
    output_file.write("-- Query in SQL:\n")
    output_file.write(
        "-- SELECT avg(tbl5_fact.col2), sum(tbl5_dim2.col2) FROM tbl5_fact,tbl5_dim2 WHERE tbl5_fact.col4=tbl5_dim2.col1 AND tbl5_fact.col2 < {} AND tbl5_dim2.col1<{};\n".format(
            int((dataSizeFact / 5) * selectivityFact),
            int(selectivityDim2 * dataSizeDim2),
        )
    )
...
def _perf_test_helper(
    output_file, dataSizeSelect, selectivity_1, selectivity_2, join_type: str
):
    ...
    output_file.write("-- Query in SQL:\n")
    output_file.write(
        "-- SELECT sum(tbl5_sel1.col1), avg(tbl5_sel2.col2) FROM tbl5_sel1, tbl5_sel2 WHERE tbl5_sel1.col1=tbl5_sel2.col1 AND tbl5_sel1.col1 < {} AND tbl5_sel2.col2<{};\n".format(
...
def create_join_correctness_test(
    ...
    # First join
    output_file_1, exp_output_file_1 = data_gen_utils.openFileHandles(
        test_num, TEST_DIR=TEST_BASE_DIR
    )
    _perf_test_helper(
        output_file_1, data_size, selectivity_1, selectivity_2, join_type_1
    )
    ...
    # Second join
    output_file_2, exp_output_file_2 = data_gen_utils.openFileHandles(
        test_num + 1, TEST_DIR=TEST_BASE_DIR
    )
    _perf_test_helper(
        output_file_2, data_size, selectivity_1, selectivity_2, join_type_2
    )
    ...
    joined_table = pre_join_sel_1.merge(
        pre_join_sel_2, left_on="col1", right_on="col1", suffixes=("", "_right")
    )
...
def createTest52_55(select_table_1, select_table_2, data_size):
    ...
    create_join_perf_test(
        data_size, selectivity_1, selectivity_2, join_type_1, join_type_2, test_num + 2
    )
...
def generateMilestoneFourFiles(
    ...
    factTable, dimTable1, dimTable2, selectTable1, selectTable2 = (
