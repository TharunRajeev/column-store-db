...
EXPERIMENT_BASE_DIR = EXPERIMENT_BASE_DATA_DIR + "milestone3"
...
def generateDataMilestone3(dataSize):
    ...
    outputTable.to_csv(
        ...
        sep=",",
        index=False,
        header=header_line_btree,
        lineterminator="\n",
    )
    outputTable.to_csv(
        outputFile_clustered_btree,
        sep=",",
        index=False,
        header=header_line_clustered_btree,
        lineterminator="\n",
    )
...
def createTests22And23(dataTable, dataSize):
    output_file22, exp_output_file22 = data_gen_utils.openFileHandles(
...
def createTest24(dataTable, dataSize):
    ...
    output_file.write(
        "-- SELECT sum(col1) FROM tbl4 WHERE (col3 >= {} and col3 < {}) AND (col2 >= {} and col2 < {});\n".format(
    ...
    dfTotalMask = (
        dfSelectMask1Low & dfSelectMask1High & dfSelectMask2Low & dfSelectMask2High
    )
...
def createTests25And26(dataTable, dataSize):
    output_file25, exp_output_file25 = data_gen_utils.openFileHandles(
...
def createTests28And29(dataTable, dataSize):
    output_file28, exp_output_file28 = data_gen_utils.openFileHandles(
        28, TEST_DIR=TEST_BASE_DIR
    )
    output_file29, exp_output_file29 = data_gen_utils.openFileHandles(
...
def createTest30():
    ...
    output_file.write("-- Load data immediately in the form of a clustered index\n")
    output_file.write(
        'load("' + DOCKER_TEST_BASE_DIR + '/data4_clustered_btree.csv")\n'
    )
    output_file.write("--\n")
    output_file.write(
        "-- Testing that the data and their indexes are durable on disk.\n"
...
def createTest32(dataTable, dataSize):
    ...
    for i in range(5):
        val1 = np.random.randint(0, int((dataSize / 5) - offset))
        output_file.write(
            "s{}=select(db1.tbl4_clustered_btree.col2,{},{})\n".format(
                i, val1, val1 + offset
            )
        )
...
def createTest33To38(dataTable, dataSize):
    ...
    for offset, selectivity in zip(offsets, selectivites):
        for table_name in table_names:
            output_file, exp_output_file = data_gen_utils.openFileHandles(
                test_start, TEST_DIR=TEST_BASE_DIR
            )
            ...
            output_file.write("-- Query in SQL:\n")
            output_file.write(
                "-- SELECT avg(col1) FROM {} WHERE col3 >= _ and col3 < _;\n".format(
                    table_name
                )
            )
            output_file.write("--\n")
            ...
            for i in range(20):
                ...
                dfSelectMask = (dataTable["col3"] >= val) & (
                    dataTable["col3"] < (val + offset)
                )
...
def createTest39To44(dataTable, dataSize):
    ...
    for offset, selectivity in zip(offsets, selectivites):
        for table_name in table_names:
            ...
            output_file.write("-- Query in SQL:\n")
            output_file.write(
                "-- SELECT avg(col3) FROM {} WHERE col2 >= _ and col2 < _;\n".format(
                    table_name
                )
            )
            output_file.write("--\n")
            ...
            for i in range(20):
                ...
                dfSelectMask = (dataTable["col2"] >= val) & (
                    dataTable["col2"] < (val + offset)
                )
...
def generateMilestoneThreeFiles(dataSize, randomSeed=47):
    ...
    if not IN_EXPERIMENTATION:
        return

    # if in experimentation mode, swith TEST_BASE_DIR to EXPERIMENT_BASE_DIR
    # then re-create test 33-38 and 39-44
    # We could copy the tests, but later we might need to play with selectivity
    # in which case we would need to re-generate the tests
    print(
        "In experimentation mode, re-creating tests 33-38 and 39-44 into"
        f" {EXPERIMENT_BASE_DIR}"
    )
    # ensure we start with a fresh experiment directory

    if os.path.exists(EXPERIMENT_BASE_DIR):
        os.system(f"rm -rf {EXPERIMENT_BASE_DIR}")
    os.makedirs(EXPERIMENT_BASE_DIR)

    global TEST_BASE_DIR
    TEST_BASE_DIR = EXPERIMENT_BASE_DIR
    createTest33To38(dataTable, dataSize)
    createTest39To44(dataTable, dataSize)
