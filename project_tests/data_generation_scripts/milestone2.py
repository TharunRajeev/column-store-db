...
from experiments2 import (
    create_tests_for_batching_impact_multicore,
    create_tests_for_batching_impact_singlecore,
)
...
# For this milestone's experimentation, we will vary the batch size from
# 2^0 to 2^8
EXPERIMENT_BASE_DIR = EXPERIMENT_BASE_DATA_DIR + "milestone2"
BATCH_SIZE_EXP_START = 0
BATCH_SIZE_EXP_END = 8
...
def createTestFourteen(dataTable):
    ...
    for i in range(10):
        dfSelectMask = (dataTable["col4"] >= (1000 * i)) & (
            dataTable["col4"] < ((1000 * i) + 30)
        )
...
def createTestFifteen(dataTable):
    ...
    # generate expected results
    for i in range(10):
        dfSelectMask = (dataTable["col4"] >= (randomVal + (2 * i))) & (
            dataTable["col4"] < (randomVal + 60 - (2 * i))
        )
        output = dataTable[dfSelectMask]["col1"]
        exp_output_file.write(data_gen_utils.outputPrint(output))
        exp_output_file.write("\n\n")
...
def generateMilestoneTwoFiles(dataSize, randomSeed):
    ...
    print("Generating tests for batching impact")

    query_starts = create_tests_for_batching_impact_multicore(
        dataTable, dataSize, 16, 17, 100, TEST_BASE_DIR
    )
    create_tests_for_batching_impact_singlecore(
        dataTable, dataSize, query_starts, 18, 19, 100, TEST_BASE_DIR
    )
    ...
    print(
        "Generating experimentation data with varying batch sizes since we"
        " are in experimentation mode. These will be stored in "
        f" {EXPERIMENT_BASE_DIR}"
    )

    # Make a fresh directory for the experimentation data
    if os.path.exists(EXPERIMENT_BASE_DIR):
        os.system(f"rm -rf {EXPERIMENT_BASE_DIR}")
    os.makedirs(EXPERIMENT_BASE_DIR)

    for i in range(BATCH_SIZE_EXP_START, BATCH_SIZE_EXP_END + 1):
        batch_size = 2**i

        target_dir = f"{EXPERIMENT_BASE_DIR}/batch_size_{batch_size}"
